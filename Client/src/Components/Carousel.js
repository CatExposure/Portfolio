...
function Carousel(){
    ...
    return (
        <div id="carousel" className="relative flex justify-center w-full">
            ...
            <button className="absolute w-8 ml-2 left-0 inset-y-0 rounded-[50%] p-[-1] h-fit my-auto text-white z-9" onClick={()=>{stopSlider(); slideLeft()}}><ArrowLeftCircleIcon/></button>
            ...
            {carImages.map((img, index) => {
                ...
                <img onMouseEnter={()=>stopSlider()} onMouseLeave={()=>startSlider()} className={`${slide===1 ? "translate-x-[-100%]" : ""} ${slide===2 ? "translate-x-[-200%]" : ""} shrink-0 grow-0 [transition:transform_800ms] w-full`} src={img.src} alt={img.alt} key={index}></img>)})}
                    <button className="absolute w-8 mr-2 right-0 inset-y-0 rounded-[50%] h-fit my-auto text-white z-9" onClick={()=>{stopSlider(); slideRight()}}><ArrowRightCircleIcon/></button>
                </div>
